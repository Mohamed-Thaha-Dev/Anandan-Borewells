...
import { Phone, CheckCircle, Zap, Droplet, Gauge, Clock, Target, Shield, Wrench, Layers, MapPin, Factory, Home, Sprout, Search, Drill } from 'lucide-react';
...
const getServiceWithIcons = (slug: string) => {
    ...
    return {
        ...data,
        whatsIncluded: data.whatsIncluded?.map((item: { text: string }) => ({ ...item, icon: CheckCircle })),
        ...
        capabilities: data.capabilities?.map((item: { title: string; description: string }) => ({ ...item, icon: getIcon(item.title.includes('Diameter') ? 'Layers' : item.title.includes('Deep') ? 'Gauge' : item.title.includes('Soil') ? 'MapPin' : 'Clock') })),
        applications: data.applications?.map((item: { title: string; description: string }) => ({ ...item, icon: getIcon(item.title.includes('Res') ? 'Home' : item.title.includes('Agri') ? 'Sprout' : 'Factory') })),
        benefits: data.benefits?.map((item: { title: string; description: string }) => ({ ...item, icon: getIcon(item.title.includes('Deep') ? 'Droplet' : item.title.includes('Flow') ? 'Gauge' : item.title.includes('Quality') ? 'Shield' : 'Wrench') })),
        whenNeeded: data.whenNeeded?.map((item: { title: string; description: string }) => ({ ...item, icon: CheckCircle })),
        methods: data.methods?.map((item: { text: string }) => ({ ...item, icon: CheckCircle }))
    };
};

// How We Work Section Component with Animation
function HowWeWorkSection({ steps }: { steps: Array<{ number: string; title: string; description: string }> }) {
        ...
        <section className="py-20 bg-white overflow-hidden">
            <div className="container mx-auto px-4 text-center">
                ...
                <p className="text-slate-500 mb-16 max-w-2xl mx-auto text-sm">
                    Our step-by-step processing ensures professional service and reliable results.
                </p>
                ...
                <div className="flex flex-col md:flex-row items-center md:items-start justify-center w-full gap-8 md:gap-0">
                    {steps.map((step, index) => {
                        ...
                        return (
                            <div key={step.number} className="flex flex-col md:flex-row items-center relative w-full md:w-auto">
                                ...
                                <div className="flex flex-col items-center relative z-10 w-full md:w-auto">
                                    ...
                                    {/* Text Content */}
                                    <div className={`mt-6 text-center max-w-[240px] md:max-w-[200px] transition-opacity duration-300 ${index + 1 === currentStep ? 'opacity-100' : 'opacity-80'}`}>
        ...
        <main>
                ...
                <div className="container mx-auto px-4 max-w-6xl">
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-10 items-start">
                        ...
                        <div className="space-y-5 max-w-lg">
                            {/* Small Square Image */}
                            <div className="">
                               <div className="rounded-lg overflow-hidden border-[3px] border-slate-200 w-full h-[200px] max-w-[250px]">
                                    <img
                        ...
                        <div>
                            {service.whatsIncluded && (
                                <div className="bg-slate-50 rounded-lg p-7">
                                    <h3 className="text-[21px] font-bold text-slate-900 mb-5">What's Included</h3>
                                    <div className="space-y-3.5">
                                        {service.whatsIncluded.map((item: any, index: number) => (
                                            <div key={index} className="flex items-start gap-3">
                                                <item.icon className="w-5 h-5 text-[#005f99] flex-shrink-0 mt-0.5" strokeWidth={2.5} />
                                                <span className="text-slate-700 text-[16px] leading-relaxed">{item.text}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {service.methods && (
                                <div className="bg-slate-50 rounded-lg p-6 mt-6">
                                    <h3 className="text-xl font-bold text-slate-900 mb-5">Our Cleaning Methods</h3>
                                    <div className="space-y-3">
                                        {service.methods.map((item: any, index: number) => (
                                            <div key={index} className="flex items-start gap-3">
                                                <div className="flex-shrink-0 mt-0.5">
                                                    <item.icon className="w-5 h-5 text-[#005f99]" strokeWidth={2.5} />
                                                </div>
                                                <span className="text-[15px] text-slate-800">{item.text}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                ...
                <section className="py-16 bg-slate-50">
                    <div className="container mx-auto px-4">
                        ...
                        <p className="text-center text-slate-600 mb-12 max-w-2xl mx-auto">
                            {service.capabilities && 'Our modern equipment and skilled team can handle any drilling requirement'}
                            {service.benefits && ''}
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
                            {(service.features || service.capabilities || service.benefits)?.map((item: any, index: number) => (
                                <ServiceFeatureCard
                                    key={index}
                                    icon={item.icon}
                                    title={item.title}
                ...
                <section className="py-20 bg-white">
                    <div className="container mx-auto px-4">
                        ...
                        <p className="text-center text-slate-500 mb-16 max-w-2xl mx-auto text-base">
                            We install various capacities based on your borewell depth and water needs
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
                            {service.motorOptions.map((option: { hp: string; type: string }, index: number) => (
                                <MotorOption
                                    key={index}
                                    hp={option.hp}
                                    type={option.type}
            ...
            {service.applications && (
                <section className="py-24 bg-white">
                    <div className="container mx-auto px-4">
                        <h2 className="text-3xl md:text-4xl font-bold text-center text-slate-900 mb-4">Applications</h2>
                        <p className="text-center text-slate-500 mb-16 max-w-2xl mx-auto text-base">
                            Our borewell drilling services cater to various needs
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                            {service.applications.map((app: { title: string; description: string }, index: number) => (
                                <div key={index} className="bg-white rounded-xl p-8 text-left border border-slate-100 shadow-sm">
                                    <h3 className="text-2xl font-bold text-slate-900 mb-4">{app.title}</h3>
                                    <p className="text-[15px] text-slate-600 leading-relaxed font-medium">{app.description}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <section className="py-20 bg-[#F4F7FA]">
                    <div className="container mx-auto px-4">
                        <h2 className="text-2xl md:text-4xl font-bold text-center text-slate-900 mb-16">
                            When Do You Need Compressor Work?
                        </h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                            {service.whenNeeded.map((item: { title: string; description: string }, index: number) => (
                                <div
                                    key={index}
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex items-start gap-5">
                                        <div className="flex-shrink-0 mt-1">
                                            <CheckCircle className="w-8 h-8 text-[#003B73]" strokeWidth={2.5} />
                                        </div>
                                        <div>
            ...
            {service.schedule && (
                <section className="py-20 bg-gradient-to-b from-[#005f99] to-[#004e8d]">
                    <div className="container mx-auto px-4 text-center">
                        <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 font-outfit">Recommended Cleaning Schedule</h2>
                        <p className="text-white/90 mb-12 max-w-3xl mx-auto text-[17px] leading-relaxed">
                            We recommend professional borewell cleaning every 2-3 years, or annually for
                            <br className="hidden md:block" /> borewells with heavy usage or poor water quality areas.
                        </p>

                        <div className="inline-block bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 px-10 shadow-xl">
                            <div className="text-1xl md:text-2xl font-bold text-[#FFC107] mb-2 font-outfit">Every 2-3 Years</div>
                            <div className="text-[13px] font-medium text-white/90">For optimal performance</div>
                        </div>
                    </div>
                </section>
            )}
            ...
            {service.signs && (
                <section className="py-24 bg-white">
                    <div className="container mx-auto px-4">
                        <h2 className="text-2xl md:text-[34px] font-bold text-center text-[#0F172A] mb-4 font-outfit">Signs Your Borewell Needs Cleaning</h2>
                        <p className="text-center text-slate-500 mb-20 max-w-2xl mx-auto text-[16px]">
                            Watch for these warning signs that indicate it's time for professional cleaning
                        </p>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-14 max-w-6xl mx-auto">
                            {service.signs.map((sign: { number: string; title: string; description: string }, index: number) => (
                                <div
                                    key={index}
            ...
            <section className="py-20 bg-gradient-to-r from-[#1e5a96] via-[#0066b3] to-[#1e8db3]">
                <div className="container mx-auto px-4 text-center">
                    <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">{service.ctaTitle}</h2>
                    ...
                    <div className="flex gap-4 justify-center flex-wrap">
                        ...
                        <a href="https://wa.me/919840105134" target="_blank" rel="noopener noreferrer">
                            <Button size="lg" className="rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white border-none font-bold px-10 py-4 text-lg shadow-xl btn-custom-hover">
